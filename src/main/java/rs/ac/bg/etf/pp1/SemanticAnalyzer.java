...
public class SemanticAnalyzer extends VisitorAdaptor {
    ...
    public void visit(ConstAssign_Bool constAssignBool) {
        constAssign(constAssignBool.getI1(), TabUtils.boolType, constAssignBool.getB2() ? 1 : 0, constAssignBool);
    }
    ...
    /*
     * VarDecl rule is used in the following places:
     * - Global variable declarations
     * - Local variable declarations (in methods)
     * - Class field declarations
     *
     * The last one is a field, and not a variable, so we have to handle
     * that case differently.
     *
     * You are not allowed to redefine a field inside a derived class
     */
    ...
    public void visit(VarName_NonArray varName) {
        ...
        if (currentClass != null && currentMethod == null) {
            // If currentMethod is not null, we are inside a class method definition
    ...
    public void visit(MethodName methodName) {
        ...
        if (methodObj != null) {
            ...
            if (currentClass != null || currentInterface != null) {
                ...
                var thisParamObj = Tab.insert(Obj.Var, THIS_VARIABLE_NAME, Tab.noType);
    ...
    private Obj completeCurrentMethod(SyntaxNode methodDecl, boolean canBeMain) {
        ...
        if (!currentMethod.getType().equals(Tab.noType) && !hasMethodReturned) {
            report_error(String.format("Non-void method '%s' must contain a return statement", currentMethod.getName()), methodDecl);
        }
    ...
    public void visit(ClassDecl_Derived classDecl) {
        ...
        // Check if interface has been implemented correctly and add non-overridden concrete methods
        for (var interfaceStruct : currentClass.getType().getImplementedInterfaces()) {
            for (var baseMethod : interfaceStruct.getMembers()) {
                var method = Tab.currentScope.findSymbol(baseMethod.getName());

                if (method == null) {
                    if (baseMethod.getFpPos() == MethodTypes.LOCAL_UNIMPLEMENTED.value) {
                        report_error(String.format("Interface method '%s' must be implemented", baseMethod.getName()), classDecl);
                    } else {
                        Tab.currentScope.addToLocals(baseMethod);
                    }
                } else if (!hasMatchingOverride(baseMethod, method, overridingType)) {
    ...
    public void visit(Designator_Ident designator) {
        ...
        if (designator.getI1().equals(THIS_VARIABLE_NAME) && currentOwner != null) {
            ...
            designator.obj = new Obj(Obj.Var, THIS_VARIABLE_NAME, thisType);
    ...
    public void visit(Designator_ArrayAccess designator) {
        ...
        if (currentObj == Tab.noObj) {
            designator.obj = Tab.noObj;
        } else if (currentObj.getKind() != Obj.Var && currentObj.getKind() != Obj.Fld || currentObj.getType().getKind() != Struct.Array) {
            ...
            designator.obj = new Obj(Obj.Elem, String.format("%s[ind]", currentObj.getName()), currentObj.getType().getElemType());
        }
    }
    ...
    public void visit(CondFact_RelopExpr condFact) {
        ...
        if ((TabUtils.isRefType(firstOperandStruct) || TabUtils.isRefType(secondOperandStruct) ||
            ...
            report_error("Only '==' and '!=' relational operators can be used with class and array variables", condFact);
            condFact.struct = Tab.noType;
        } else if (!TabUtils.compatibleWith(firstOperandStruct, secondOperandStruct)) {
            report_error("Attempt to compare variables of non-compatible types", condFact);
            condFact.struct = Tab.noType;
        } else {
    ...
    public void visit(DesignatorStatement_AssignExpr designatorStatement) {
        ...
        } else if (!TabUtils.assignableTo(designatorObj.getType(), designatorStatement.getExpr().struct)) {
            report_error(String.format("Assignment of a value with incompatible type to '%s'", designatorObj.getName()), designatorStatement);
        }
    }
    ...
    public void visit(DesignatorStatement_Inc designatorStatement) {
        var designatorObj = designatorStatement.getDesignator().obj;
        if (designatorObj.equals(Tab.noObj)) return;

        if (designatorObj.getKind() != Obj.Var && designatorObj.getKind() != Obj.Elem && designatorObj.getKind() != Obj.Fld) {
            report_error(String.format("Attempt to increment a non-variable '%s'", designatorObj.getName()), designatorStatement);
        } else if (!designatorObj.getType().equals(Tab.intType)) {
            report_error(String.format("Attempt to increment a non-integer variable '%s'", designatorObj.getName()), designatorStatement);
        }
    }
    ...
    public void visit(DesignatorStatement_Dec designatorStatement) {
        ...
        if (designatorObj.getKind() != Obj.Var && designatorObj.getKind() != Obj.Elem && designatorObj.getKind() != Obj.Fld) {
            report_error(String.format("Attempt to decrement a non-variable '%s'", designatorObj.getName()), designatorStatement);
        } else if (!designatorObj.getType().equals(Tab.intType)) {
            report_error(String.format("Attempt to decrement a non-integer variable '%s'", designatorObj.getName()), designatorStatement);
        }
    }
    ...
    public void visit(DesignatorStatement_AssignSetop designatorStatement) {
        ...
        if (!firstOperandObj.getType().equals(TabUtils.setType) || !secondOperandObj.getType().equals(TabUtils.setType)) {
            ...
            report_error("Result of the set operator can only be assigned to a variable of set type", designatorStatement);
        }
    }
    ...
    public void visit(Statement_Read statement) {
        ...
        if (statementObj.getKind() != Obj.Var && statementObj.getKind() != Obj.Elem && statementObj.getKind() != Obj.Fld) {
            report_error(String.format("Attempt to read into a non variable '%s'", statementObj.getName()), statement);
            ...
            report_error(String.format("Attempt to read into a variable '%s' of a non-compatible type", statementObj.getName()), statement);
        }
    }
    ...
    public void visit(Statement_PrintExprWithNum statement) {
        if (!validatePrintStatementExpr(statement.getExpr().struct)) {
            report_error("Attempt to print a variable of a non-compatible type", statement);
        }
    }
    ...
    public void visit(Statement_ReturnExpr statement) {
        if (currentMethod != null && !TabUtils.assignableTo(currentMethod.getType(), statement.getExpr().struct)) {
            report_error(String.format("Type of return value doesn't match the return type of method '%s'", currentMethod.getName()), statement);
        } else {
    ...
    private Struct validateCall(CallableRef callableRef, Struct argsStruct, SyntaxNode node) {
        ...
        if (method.getKind() != Obj.Meth) {
            report_error(String.format("Attempt to call a non-method '%s'", method.getName()), node);
            return Tab.noType;
        }
    ...
    private Obj getSymbolObj(String symbolName) {
        ...
        if (Tab.currentScope.equals(programScope)) {
            // If name is inside top (program) scope, check universe scope as well
